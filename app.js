...
const banks = [
  {
    name:"Сбер — прямой tel:",
    directHref:`tel:${phone}`,
    sberDiagnostic:true
  },
  {
    name:"Сбер — browser intent",
    directHref:
...
holder.innerHTML = "";
banks.forEach(bank=>{
  // Для двух Sber-тестов используем настоящий <a href>, а не location.href из JS.
  // Это важно: Chrome рекомендует intent-ссылку запускать непосредственно
  // пользовательским нажатием на anchor.
  if (bank.directHref) {
