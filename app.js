...
const banks = [
  {
    name:"СберБанк",
    // Подтверждённый рабочий Android browser intent:
    // открывает Сбер и передаёт номер телефона.
    // Сумма пока вводится пользователем вручную.
    directHref:
      `intent:${phone}#Intent;scheme=tel;action=android.intent.action.VIEW;` +
      `category=android.intent.category.BROWSABLE;package=ru.sberbankmobile;end`,
    androidOnly:true
  },
  {
    name:"Т-Банк",
...
banks.forEach(bank=>{
  if (bank.directHref) {
    ...
    link.className = "bankBtn";
    link.href = bank.directHref;
    link.innerHTML =
    ...
    link.addEventListener("click", (e)=>{
      ...
      // Android — разрешаем браузеру открыть подтверждённый Sber intent.
      // Desktop — intent не имеет смысла, поэтому не оставляем "молчаливую" кнопку.
      if (bank.androidOnly && !/Android/i.test(navigator.userAgent)) {
        e.preventDefault();
        showToast("Номер скопирован. На компьютере откройте Сбер вручную.");
      }
    });
