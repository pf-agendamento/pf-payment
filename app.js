const PAYMENT_LANDING_VERSION = "0.6.1-netqr1";
  ...
  qrLibraryPromise = new Promise((resolve, reject) => {
    ...
    script.onload = () => {
      if (window.QRCode && typeof window.QRCode.toDataURL === "function") {
        ...
        reject(new Error("QR library loaded without QRCode API"));
      }
    };
    script.onerror = () => reject(new Error("Не удалось загрузить локальный QR-генератор."));
...
async function makeBankSpecificQrDataUrl(bankUrl) {
  const QRCodeApi = await loadQrLibrary();

  return QRCodeApi.toDataURL(bankUrl, {
...
async function showPaymentReadyDesktop(bankName, bankUrl) {
  try {
    const qrDataUrl = await makeBankSpecificQrDataUrl(bankUrl);
    ...
    openModal(`
      ...
      <div class="qrWrap">
        <img
          src="${qrDataUrl}"
          alt="QR-код для открытия ${bankName}"
