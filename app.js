...
const banks = [
  {
    name:"СберБанк",
    // Проверенный Android-маршрут:
    // ACTION_VIEW + tel:<номер> + package ru.sberbankmobile.
    // В browser intent URI схема tel передаёт номер прямо приложению Сбера.
    sberPhoneIntent:true
  },
  {
    name:"Т-Банк",
    links:tbankLinks(),
...
function openBank(bank){
  ...
  // Sber baseline:
  // эквивалент проверенного ADB-вызова
  // VIEW tel:<phone> в package ru.sberbankmobile.
  // Сумма здесь намеренно НЕ передаётся: пользователь вводит её вручную.
  if (bank.sberPhoneIntent) {
    const fallback = encodeURIComponent(location.href + "#sber-app-not-opened");
    const intent =
      `intent://${phone}#Intent;scheme=tel;package=ru.sberbankmobile;` +
